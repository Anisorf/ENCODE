...
#macro( ok_service_plain_text $arg )##
#if( ! $arg )#set( $arg = "" )#end##
...
#set( $okEventSI = "http://omakaupunki.fi/event/" )##
#set( $okEventItem = $topicmap.getTopic( $okEventSI ) )##

#set( $okLocationSi = "http://www.geonames.org/coordinates" )##
#set( $okLocationItem = $topicmap.getTopic( $okLocationSi ) )##
#if( $okLocationItem )##
    #set( $ok_events = $topicmap.getTopicsOfType( $okEventItem ) )##
#end##
...
var ok_event_locations = [
#if( $ok_events && $ok_events.size()!=0 )##
	#foreach( $event in $ok_events )##
	{
		#set( $location = $event.getData( $okLocationItem, $lang ) )##
		#set ( $title = $event.getDisplayName($lang) )##
		title: #ok_service_plain_text($title),
		location: #ok_service_plain_text( $location ),
		...
		#foreach( $layer in $associations )##
			...
			#foreach($role in $roles) ##
				...
				#if(!$tmbox.isTopicOfType($player, $okEventItem) )##
		...
		#set( $data_types = $event.getDataTypes() )##
			...
			#foreach( $type in $data_types )##
				...
				{
				name:#ok_service_plain_text($type.getDisplayName($lang) ),
				values:[#ok_service_plain_text( $event.getData($type, $lang) )]
				},
				#end##
			#end##
		#end##
		...
		{
			name: #ok_service_plain_text($entry.key),
			values: [
			#foreach($value in $entry.value)##
			#ok_service_plain_text($value),
			#end##
			]
		},
		#end
...
for(var i=0;i<ok_event_locations.length;i++) {

	var raw_latlng = ok_event_locations[i].location.split(",");
	var latlng = new google.maps.LatLng(raw_latlng[0],raw_latlng[1]);
	
	var loc_data = ok_event_locations[i];
