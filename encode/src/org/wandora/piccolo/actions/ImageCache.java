...
public class ImageCache implements Action,XMLParamAware {
    
    private Logger logger;
    private String cachedir;
    private HashMap<String,ReaderWriterLock> locks;
    private HashMap<String,Profile> profiles;
    
    private int FLAG_CROP=1;
    ...
    public ImageCache() {
        locks=new HashMap<String,ReaderWriterLock>();
                    ...
                    for(int j=0;j<ss.getLength();j++){
                        ...
                        if(ssn instanceof Element) {
                            ...
                            if(sse.getNodeName().equals("authorizer")) {
                                ...
                                secondaryUrl = processor.getElementContents(sse);
                            }
                            else if(sse.getNodeName().equals("savedir")) {
                                secondarySavedir = processor.getElementContents(sse);
                            }
                        }
                    }
                }
    ...
    protected String encodeFileName(String url,Profile profile){
        ...
        String dir=cachedir;
        if(profile.cacheDir!=null) dir=profile.cacheDir;
        return dir+s+"-"+profile.id;
    }
    ...
    /**
     * Gets a reader writer lock for the given file. Does not use the lock
     * to lock the file.
     */
    protected synchronized ReaderWriterLock getLock(String filename){
        ReaderWriterLock lock=locks.get(filename);
        if(lock==null) {
    ...
    protected void sendFile(String filename,OutputStream out) throws IOException {
        ReaderWriterLock lock=getLock(filename);
        if(lock.getReaderLock()){
            try{
...
/*                
     ...
     * the method returns valid image. Othervise an exception is thrown.
     *
...
//                logger.writelog("DBG","Using cached image for "+url);                
    ...
    public static interface ImageMaker {
         ...
         */
        public BufferedImage makeImg(ImageCache cache,BufferedImage original);
    }
