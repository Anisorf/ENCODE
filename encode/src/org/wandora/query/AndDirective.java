 ...
 * AndDirective.java
 *
 * Created on 29. lokakuuta 2007, 13:15
 *
 */
 ...
 * @author olli
 */
public class AndDirective extends FilterDirective {
    
    private FilterDirective[] filters;
    
    /** Creates a new instance of AndDirective */
    public AndDirective(Directive query,boolean not,FilterDirective ... filters) {
        super(query,not);
        this.filters=filters;
    }
    public AndDirective(Directive query,boolean not,FilterDirective filter1) {
        this(query,not,new FilterDirective[]{filter1});
    }
    public AndDirective(Directive query,boolean not,FilterDirective filter1,FilterDirective filter2) {
        this(query,not,new FilterDirective[]{filter1,filter2});
    }
    public AndDirective(Directive query,boolean not,FilterDirective filter1,FilterDirective filter2,FilterDirective filter3) {
        this(query,not,new FilterDirective[]{filter1,filter2,filter3});
    }
    public AndDirective(Directive query,boolean not,FilterDirective filter1,FilterDirective filter2,FilterDirective filter3,FilterDirective filter4) {
    ...
    protected int _includeRow(ResultRow row, Topic context, TopicMap tm, Object param) throws TopicMapException {
        ...
        for(int i=0;i<filters.length;i++){
            ...
            if(r==RES_EXCLUDE) return RES_EXCLUDE;
            else if(r==RES_INCLUDE) ignore=false;
        }
        if(ignore) return RES_IGNORE;
        else return RES_INCLUDE;
    }
