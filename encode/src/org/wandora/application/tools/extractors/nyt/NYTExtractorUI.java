...
public class NYTExtractorUI extends javax.swing.JPanel {
    ...
    public WandoraTool[] getExtractors(NYTExtractor tool) throws TopicMapException {
        ...
        if (articleSearchPanel.equals(component)) {
            ...
            String extractUrl = NYT_API_BASE + "articlesearch.json?q=" + urlEncode(query) + "&api-key=" + key;

            String facets = facetsTextField.getText().trim();
            if (facets != null && facets.length() > 0) {
                extractUrl += "&facet_field=" + urlEncode(facets);
            }
            ...
            if (fieldValues.length() > 0) {
                extractUrl += "&f1=" + urlEncode("url," + fieldValues.toString());
            }
            ...
            String offset = pageTextField.getText().trim();
            if (offset == null || offset.length() == 0) {
                offset = "0";
            }
            extractUrl += "&page=" + urlEncode(offset);

            String rank = sortComboBox.getSelectedItem().toString();
    ...
    private void initComponents() {
        ...
        pageLabel.setToolTipText("The value of offset corresponds to a set of 10 results.");
        gridBagConstraints = new java.awt.GridBagConstraints();
        gridBagConstraints.gridx = 0;
        gridBagConstraints.anchor = java.awt.GridBagConstraints.LINE_END;
        gridBagConstraints.insets = new java.awt.Insets(0, 0, 2, 4);
        optionalArticleSearchFieldsPanel.add(pageLabel, gridBagConstraints);
        gridBagConstraints = new java.awt.GridBagConstraints();
        gridBagConstraints.gridx = 1;
        gridBagConstraints.fill = java.awt.GridBagConstraints.HORIZONTAL;
        gridBagConstraints.weightx = 1.0;
        gridBagConstraints.insets = new java.awt.Insets(0, 0, 2, 0);
        optionalArticleSearchFieldsPanel.add(pageTextField, gridBagConstraints);

        sortLabel.setText("Sort");
        sortLabel.setToolTipText("Use the rank parameter to set the order of the results. The default rank is newest.");
        gridBagConstraints = new java.awt.GridBagConstraints();
        gridBagConstraints.gridx = 0;
        gridBagConstraints.anchor = java.awt.GridBagConstraints.LINE_END;
        ...
        optionalArticleSearchFieldsPanel.add(sortComboBox, gridBagConstraints);
