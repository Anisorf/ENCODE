...
public class DiscogsLabelExtractor extends AbstractDiscogsExtractor {
    ...
    public String getName() {
        return "Discogs API Label extractor";
    }
    @Override
    public String getDescription(){
        return "Extractor fetches label data from Discogs API.";
    }
    ...
    public void parseLabel(JSONObject json, TopicMap tm) throws TopicMapException {
        if(json.has("results")) {
            try {
                ...
                if (resultsArray.length() > 0) {
                    ...
                    log("Search returned " + count + " labels.");
                } else {
    ...
    public void parseResult(JSONObject result, TopicMap tm) throws JSONException, TopicMapException {
        ...
        if(result.has("uri") && result.has("id")) {
            ...
            itemTopic.addType(getLabelTypeTopic(tm));
