 ...
 *
 * BingExtractorSelector.java
 *
 * Created on 19.12.2009, 12:33:39
 */
...
public class BingExtractorSelector extends javax.swing.JDialog {
    ...
    public BingExtractorSelector(Wandora wandora) {
        super(wandora, true);
        initComponents();
        setSize(450,250);
        setTitle("Bing extractors");
    ...
    public WandoraTool getWandoraTool(WandoraTool parentTool) {
        Component component = bingTabbedPane.getSelectedComponent();
        WandoraTool wt = null;
        ...
        if(searchPanel.equals(component)) {
            ...
            if(searchWords == null || searchWords.trim().length() == 0) {
                parentTool.log("No search words given. Aborting.");
                return null;
            }
            try {
            ...
            BingSearchResultExtractor bse = new BingSearchResultExtractor();
            bse.setSearchString(searchWords);
            searchWords="'" + searchWords + "'";

            HashMap<String,String> auth = bse.solveAuth(wandora);

            String uname = urlEncode(auth.get("Username"));
            String key   = urlEncode(auth.get("Account key"));

            String bingURL = "https://" + uname + ":" + key + "@" 
                    + AbstractBingExtractor.BING_URL
                    + "?Query="+searchWords
                    + "&$top=" + number
                    + "&$skip=" + offset
                    + "&$format=Atom";

            bse.setForceUrls(new String[] { bingURL } );
            wt = bse;
...
/*
    ...
    private void initComponents() {
        ...
        bingTabbedPane = new SimpleTabbedPane();
        ...
        searchLabel.setText("<html>Perform Bing search and convert search result to a topic map. Write search query to the field below.</html>");
        ...
        bingTabbedPane.addTab("Search", searchPanel);

        gridBagConstraints = new java.awt.GridBagConstraints();
        gridBagConstraints.fill = java.awt.GridBagConstraints.BOTH;
        gridBagConstraints.weightx = 1.0;
        gridBagConstraints.weighty = 1.0;
        tabbedPanel.add(bingTabbedPane, gridBagConstraints);
    ...
    private javax.swing.JTabbedPane bingTabbedPane;
